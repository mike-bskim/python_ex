...
class FakeDataStore:
    ...
    # 변수 업데이트 함수
    def update(self, n):
        logging.info('Thread {}: starting update local_copy({})'.format(n, self.value))
        ...
        logging.info('Thread {}: ending update local_copy({})'.format(n, self.value))
...
def main():
    ...
    logging.info('Testing update. Starting value is ({})'.format(store.value))
    ...
    logging.info('Testing update. Ending value is ({})'.format(store.value))
