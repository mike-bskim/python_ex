...
동시성(장점) : 논리적, 동시 실행 패턴(논리적), 싱글코어, 멀티 코어에서 실행 가능, 한 개의 작업 공유 처리, 
동시성(단점) : 디버깅 매우 어려움, Mutex, Deadlock

병렬성(장점) : 물리적, 물리적 동시 실행, 멀티 코어에서 구현 가능, 주로 별개의 작업 처리, 
병렬성(단점) : 디버깅 어려움, OpenMP, MPI, CUDA
